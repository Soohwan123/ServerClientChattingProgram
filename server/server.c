    ...
    while (1) {
        ...
        for (int i = 0; i < n_ready; i++) {
            ...
            if (events[i].data.fd == server_socket) {
                ...
                event.events = EPOLLIN; // 입력 이벤트 설정
            ...
            // 클라이언트 데이터 처리
                ...
                while ((bytes_read = read(client_fd, buffer, sizeof(buffer))) > 0) {
                    ...
                    if (strstr(buffer, "Upgrade: websocket")) {
                        ...
                        if (websocket_key) {
                            websocket_handshake(client_fd, websocket_key);
                        } else {
                    ...
                    // WebSocket 메시지 처리
                        ...
                        decode_websocket_frame(buffer, bytes_read, decoded_message, sizeof(decoded_message));
